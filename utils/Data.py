...
class Data(object):
    ...
    def __new_min_bar__(self, bar):
        ...
        bar_time = time.strptime(
            '{0}-{1}-{2} {3}:{4}'.format(year, mon, day, hour, mins), '%Y-%m-%d %H:%M')
        # time -> str
    ...
    def __order__(self, direction, offset, price, volume, remark):
        ...
        if self.SingleOrderOneBar and (self.LastEntryDateLong == self.D[-1] or self.LastEntryDateShort ==
                                       self.D[-1] or self.ExitDateLong == self.D[-1] or self.ExitDateShort == self.D[-1]):
            return
        order = OrderItem()
        ...
        for case in switch(diroff):
            ...
            if case('Sell-Open'):
                ...
                if self._lastOrder.PositionShort == 0:
                    ...
                    # time or double or str ???
                    order.EntryDateShort = self.D[-1]
