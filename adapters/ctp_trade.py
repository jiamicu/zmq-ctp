...
from adapters.TradeAdapter import *
...
# ------------------
        ...
        if bIsLast:
            ...
            self.__posi = sorted(
                ...
                key=lambda c: '{0}_{1}'.format(
                    c.getInstrumentID(),
                    DirectType.Buy if c.getPosiDirection() == PosiDirectionType.Long else DirectType.Sell))
            # direction需从posidiction转换为dictiontype
            for key, group in itertools.groupby(self.__posi, lambda c: '{0}_{1}'.format(c.getInstrumentID(
            ), DirectType.Buy if c.getPosiDirection() == PosiDirectionType.Long else DirectType.Sell)):
                ...
                for g in group:
                    ...
                    cost += g.OpenCost
                # pf.Position <= 0 ? 0 : (g.Sum(n => n.PositionCost) /
                # DicInstrumentField[pf.InstrumentID].VolumeMultiple /
                # pf.Position);
    ...
    def __OnRtnOrder(self, pOrder=CThostFtdcOrderField):
        ...
        if not of:
            ...
            self.OnRtnOrder(of)
            # _thread.start_new_thread(self.OnRtnOrder, (of,))  # call client
            # OnRtnOrder event
            ...
            if pOrder.getOrderSysID():
                ...
                self.__dic_orderid_sysid[pOrder.getOrderSysID()] = id
    ...
    def __OnRtnTrade(self, f):
        ...
        of.AvgPrice = (of.AvgPrice * (of.Volume - of.VolumeLeft) + \
                       tf.Price * tf.Volume) / (of.Volume - of.VolumeLeft + tf.Volume)
        ...
        if tf.Offset == OffsetType.Open:
            ...
            key = '{0}_{1}'.format(
                tf.InstrumentID,
                DirectType.Sell if tf.Direction == DirectType.Buy else DirectType.Buy)
            pf = self.DicPositionField.get(key)
            if pf:  # 有可能出现无持仓的情况
    ...
    def __OnRspOrder(
        ...
        id = '{0}|{1}|{2}'.format(
            self.SessionID, '0', pInputOrder.getOrderRef())
        of = self.DicOrderField.get(id)
        if not of:
            of = OrderField()
